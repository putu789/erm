...
if(isset($_GET['id'])) {
    $id = $_GET['id']; 
     $_sql = "SELECT a.no_rkm_medis, a.no_rawat, b.nm_pasien, b.umur,c.png_jawab,b.alamat,b.namakeluarga,d.nm_kel,e.nm_kec,f.nm_kab,a.kd_poli FROM reg_periksa a, pasien b, penjab c,kelurahan d, kecamatan e, kabupaten f WHERE a.no_rawat = '".$id."' AND a.no_rkm_medis = b.no_rkm_medis AND a.kd_pj = c.kd_pj AND  d.kd_kel = b.kd_kel And b.kd_kec = e.kd_kec And b.kd_kab = f.kd_kab";
                        ...
                        <div class="header">
                         ...
                         <main class="bg">
                            <input class="ip" id="tab1" type="radio" name="tabs" checked>
                                <label class="lb" for="tab1">Belum Diperiksa</label>
                              <input class="ip" id="tab2" type="radio" name="tabs">
                                <label class="lb" for="tab2">Sudah Diperiksa</label>
								<input class="ip" id="tab3" type="radio" name="tabs">
                                <label class="lb" for="tab3">
                                <?php
	    						$_sql = "SELECT COUNT(*) As jumlah FROM rujukan_internal_poli i, reg_periksa a, pasien b, dokter c,kelurahan e, kecamatan f, kabupaten g, penjab h  WHERE i.kd_dokter = '{$_SESSION['username']}' And i.no_rawat = a.no_rawat AND a.no_rkm_medis = b.no_rkm_medis AND a.kd_dokter = c.kd_dokter   AND b.kd_kel = e.kd_kel And b.kd_kec = f.kd_kec And a.kd_pj = h.kd_pj And b.kd_kab = g.kd_kab AND a.stts='Sudah'"; 
        						if(isset($_POST['tanggal']) && $_POST['tanggal'] !="") {
            						$_sql .= " AND a.tgl_registrasi = '{$_POST['tanggal']}'";
        						} else { 
            						$_sql .= " AND a.tgl_registrasi = '$date'";
        						}
        						$_sql .= " GROUP BY i.kd_dokter ORDER BY a.no_reg ASC";

        						$sql = query($_sql); 
        						$no = 1;
								while($row = fetch_array($sql)){
									echo '<span class="badge" style="border-radius:100%; background-color:#12ba60;">'.$row['jumlah'].'</span>';
								}?>Rujukan Internal</label>
                            ...
                            <table id="jadwal_dokter" class="table table-bordered " width="100%">
	    						...
	    						<tbody>
	    						<?php
	    						$_sql = "SELECT b.nm_pasien, c.nm_dokter, a.no_reg, a.no_rkm_medis, a.stts, a.no_rawat,b.alamat,b.namakeluarga,e.nm_kel,f.nm_kec,g.nm_kab,b.namakeluarga,h.png_jawab FROM reg_periksa a, pasien b, dokter c,kelurahan e, kecamatan f, kabupaten g, penjab h WHERE a.kd_dokter = '{$_SESSION['username']}' AND a.no_rkm_medis = b.no_rkm_medis AND a.kd_dokter = c.kd_dokter   AND b.kd_kel = e.kd_kel And b.kd_kec = f.kd_kec And a.kd_pj = h.kd_pj And b.kd_kab = g.kd_kab AND a.stts='Dirawat'"; 
        						if(isset($_POST['tanggal']) && $_POST['tanggal'] !="") {
            						$_sql .= " AND a.tgl_registrasi = '{$_POST['tanggal']}'";
        						} else { 
            						$_sql .= " AND a.tgl_registrasi = '$date'";
								...
								if ($st === "Belum"){
										...
										$btn ="<button type='button' class='view_data btn btn-primary btn-xs' data-toggle='modal' id='".$row['5']."' data-target='#data_modal'>Lihat data</button>";
		    						...
		    						echo '<td>'.$no.'</td>';
		    						echo '<td>';
		    						echo '<a href="'.$_SERVER['PHP_SELF'].'?action=view&tanggal='.$_POST['tanggal'].'&id='.$row['no_rawat'].'" class="title obt">'.ucwords(strtolower($row['0'])).'</a><span class="label label-warning"><i>'.$row['png_jawab'].'</i></span><br><i>('.$row['namakeluarga'].')</i><br>
                      	...
                      	<div class="body">
                        	<form method="POST" action="">
                             ...
                             <table id="jadwal_dokter" class="table table-bordered" width="100%">
	    						...
	    						<tbody>
	    						<?php
	    						$_sql = "SELECT b.nm_pasien, c.nm_dokter, a.no_reg, a.no_rkm_medis, a.stts, a.no_rawat,b.alamat,b.namakeluarga,e.nm_kel,f.nm_kec,g.nm_kab,b.namakeluarga FROM reg_periksa a, pasien b, dokter c,kelurahan e, kecamatan f, kabupaten g WHERE a.kd_dokter = '{$_SESSION['username']}' AND a.no_rkm_medis = b.no_rkm_medis AND a.kd_dokter = c.kd_dokter AND a.stts='Sudah'  AND b.kd_kel = e.kd_kel And b.kd_kec = f.kd_kec And b.kd_kab = g.kd_kab"; 
        						if(isset($_POST['tanggal']) && $_POST['tanggal'] !="") {
            						$_sql .= " AND a.tgl_registrasi = '{$_POST['tanggal']}'";
        						} else { 
            						$_sql .= " AND a.tgl_registrasi = '$date'";
								...
								$btn=$row['4'];
		    						...
		    						echo '<td>'.$no.'</td>';
		    						echo '<td>';
		    						echo '<a href="'.$_SERVER['PHP_SELF'].'?action=view&tanggal='.$_POST['tanggal'].'&id='.$row['no_rawat'].'" class="title obt" idob="'.$row['no_rawat'].'">'.ucwords(strtolower($row['0'])).'</a><br><i>('.$row['namakeluarga'].')</i><br>
											'.$row['alamat'].','.$row['nm_kel'].','.$row['nm_kec'].','.$row['nm_kab'].'';
		    						echo '</td>';
		    						echo '<td>'.$row['1'].'</td>';
		    						echo '<td>'.$row['2'].'</td>';
						  ...
						  <section class="section" id="content3">
                            ...
                            <table id="jadwal_dokter" class="table table-bordered " width="100%">
	    						...
	    						<tbody>
	    						<?php
	    						$_sql = "SELECT b.nm_pasien, c.nm_dokter, a.no_reg, a.no_rkm_medis, a.stts, a.no_rawat,b.alamat,b.namakeluarga,e.nm_kel,f.nm_kec,g.nm_kab,b.namakeluarga,h.png_jawab FROM reg_periksa a, pasien b, dokter c,kelurahan e, kecamatan f, kabupaten g, penjab h, rujukan_internal_poli i WHERE i.kd_dokter = '{$_SESSION['username']}' And i.no_rawat = a.no_rawat AND a.no_rkm_medis = b.no_rkm_medis AND a.kd_dokter = c.kd_dokter   AND b.kd_kel = e.kd_kel And b.kd_kec = f.kd_kec And a.kd_pj = h.kd_pj And b.kd_kab = g.kd_kab AND a.stts='Sudah'"; 
        						if(isset($_POST['tanggal']) && $_POST['tanggal'] !="") {
            						$_sql .= " AND a.tgl_registrasi = '{$_POST['tanggal']}'";
        						} else { 
            						$_sql .= " AND a.tgl_registrasi = '$date'";
								...
								if ($st === "Belum"){
		    						...
		    						echo '<td>'.$no.'</td>';
		    						echo '<td>';
		    						echo '<a href="periksa-rujukan.php?tanggal='.$_POST['tanggal'].'&id='.$row['3'].'&idob='.$row['no_rawat'].'" class="title obt">'.ucwords(strtolower($row['0'])).'</a><span class="label label-warning"><i>'.$row['png_jawab'].'</i></span><br><i>('.$row['namakeluarga'].')</i><br>
											'.$row['alamat'].','.$row['nm_kel'].','.$row['nm_kec'].','.$row['nm_kab'].'';
		    						echo '</td>';
		    						echo '<td>'.$row['1'].'</td>';
									echo '<td>'.$row['2'].'</td>';
...
<?php 
if ( $_SESSION['jenis_poli'] == "U0005"){
?>
<button type="button" class="v_mata btn btn-info" data-toggle='modal' id='<?php echo $no_rkm_medis; ?>' tgl='<?php echo $date;?>'  dok='<?php echo $_SESSION['username']; ?>' no_raw='<?php echo $no_rawat; ?>' data-target='#myModal2'>Riwayat Mata</button>
<?php 
}else{
	echo "";
}?>
 ...
 d
                      ...
                      <?php echo $angka_format ;?>&nbsp;<?php echo $st; ?></td>
                       ...
                       <td><?php echo $data_vital['riwayat'];?>, <?php echo $data_vital['riwayat_lain'];?></td>
                      
                      </tbody>
                      </table>
                     
                      </div> 
                       <?php if ($_SESSION['jenis_poli'] == "U0005"){
						 
                           $cet=num_rows(query("SELECT no_rawat FROM p_mata WHERE no_rawat='{$no_rawat}'"));
							if ($cet > 0){
								echo "<button type='button' class='cek btn btn-md btn-info' data-toggle='modal' id=".$no_rawat." data-target='#meModal1'>Lihat Pemeriksaan</button>";
							}else {
						  echo "<a href='periksa-mata/index.php?no_rawat=".$no_rawat."' target='_blank'><button type='button' class='btn btn-md btn-success'>Pemeriksaan gambar</button></a>";
							}
					  } else{
						  echo "";
					  }
