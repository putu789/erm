...
$q = $_GET['q'];
		$sql = query("SELECT databarang.kode_brng AS id, databarang.nama_brng AS text ,gudangbarang.stok AS stok
					  FROM databarang, gudangbarang 
						WHERE (databarang.kode_brng LIKE '%".$q."%' OR databarang.nama_brng LIKE '%".$q."%')
						and gudangbarang.kode_brng = databarang.kode_brng  and databarang.kode_kategori = 'A65'
						and gudangbarang.stok > 0
					");
		$json = [];
		
		while($row = fetch_assoc($sql)){
			 $json[] = ['id'=>$row['id'], 'text'=>$row['text'],'stok'=>$row['stok']];
			 }
